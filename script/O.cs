...
/// <summary>
/// Script Unity pour la lettre O en LSF.
/// Les 4 doigts courbés rejoignent le pouce pour former un cercle.
/// Position fixe, pose maintenue.
/// </summary>
public class O : MonoBehaviour
{
    ...
    private void Start()
    {
        if (playOnStart) PlayO();
    }

    public void PlayO()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayOCoroutine());
    }

    private IEnumerator PlayOCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[O] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[O] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[O] Démarrage.");
