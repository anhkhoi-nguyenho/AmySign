...
/// <summary>
/// Script Unity pour la lettre P en LSF.
/// Index + majeur tendus en diagonale, annulaire + auriculaire repliés, pouce dessus.
/// Le poignet bascule de 90° entre les deux phases.
/// </summary>
public class P : MonoBehaviour
{
    ...
    [Header("Durées des phases")]
    [Tooltip("Phase 1 : doigts en diagonale vers le haut")]
    public float phase1Duration = 0.5f;
    [Tooltip("Transition entre phase 1 et phase 2")]
    public float transitionDuration = 0.3f;
    [Tooltip("Phase 2 : doigts en diagonale vers le bas (maintenue)")]
    public float phase2Duration = 0.7f;

    [Header("Poignet")]
    public bool applyWristRotation = true;
    [Tooltip("Phase 1 : doigts en diagonale (rotation latérale du poignet)")]
    public Vector3 wristEulerDiagonaleHaut = new Vector3(0f, -180f, -45f);
    [Tooltip("Phase 2 : poignet tourne sur le côté de 90° (dans l'autre sens)")]
    public Vector3 wristEulerDiagonaleBas = new Vector3(0f, -180f, -135f);
    ...
    private void Start()
    {
        if (playOnStart) PlayP();
    }

    public void PlayP()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayPCoroutine());
    }

    private IEnumerator PlayPCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[P] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[P] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[P] Démarrage.");

        ApplyGaze();
        float safeSpeed = Mathf.Max(0.01f, globalSpeedMultiplier);

        // Phase 1 : doigts diagonale vers le haut
        if (showDebugLogs) Debug.Log("[P] Phase 1 : diagonale haut.");
        float elapsed = 0f;
        float realDur1 = phase1Duration / safeSpeed;
        while (elapsed < realDur1)
        {
            elapsed += Time.deltaTime;
            ApplyArmPose();
            if (applyWristRotation) ApplyWristRot(wristEulerDiagonaleHaut);
            if (forceManualHandshape) UpdateFingerRotations();
            yield return null;
        }

        // Transition : bascule du poignet
        if (showDebugLogs) Debug.Log("[P] Transition bascule poignet.");
        Quaternion startRot = Quaternion.Euler(wristEulerDiagonaleHaut);
        Quaternion endRot = Quaternion.Euler(wristEulerDiagonaleBas);
        elapsed = 0f;
        float realTrans = transitionDuration / safeSpeed;
        while (elapsed < realTrans)
        {
            elapsed += Time.deltaTime;
            float t = Mathf.Clamp01(elapsed / realTrans);
            float smoothT = Mathf.SmoothStep(0f, 1f, t);
            ApplyArmPose();
            if (applyWristRotation)
            {
                Quaternion currentRot = Quaternion.Slerp(startRot, endRot, smoothT);
                if (useLocalRotation) rightHandTarget.localRotation = currentRot;
                else                  rightHandTarget.rotation = currentRot;
            }
            if (forceManualHandshape) UpdateFingerRotations();
            yield return null;
        }

        // Phase 2 : doigts diagonale vers le bas (maintenue)
        if (showDebugLogs) Debug.Log("[P] Phase 2 : diagonale bas (maintenue).");
        elapsed = 0f;
        float realDur2 = phase2Duration / safeSpeed;
        while (elapsed < realDur2)
        {
            elapsed += Time.deltaTime;
            ApplyArmPose();
            if (applyWristRotation) ApplyWristRot(wristEulerDiagonaleBas);
            if (forceManualHandshape) UpdateFingerRotations();
            yield return null;
        }

        // Pose maintenue à l'infini
        while (true)
        {
            ApplyArmPose();
            if (applyWristRotation) ApplyWristRot(wristEulerDiagonaleBas);
    ...
    private void ApplyWristRot(Vector3 euler)
    {
        if (rightHandTarget == null) return;
        Quaternion r = Quaternion.Euler(euler);
