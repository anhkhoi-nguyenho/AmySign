...
/// <summary>
/// Script Unity pour la lettre I en LSF.
/// Généré depuis lettre_i_output_gestes_unity_1m60_v1.json.
/// Auriculaire tendu seul ; index, majeur, annulaire et pouce repliés.
/// Pose maintenue sans retour au repos.
/// </summary>
public class LettreIUnityPlayer_1m60_V1 : MonoBehaviour
{
    ...
    [Header("Pouce — replié")]
    public Vector3 thumbPh1 = new Vector3(40f, 20f, 40f);
    ...
    private void Start()
    {
        if (playOnStart) PlayLettreI();
    }

    public void PlayLettreI()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayLettreICoroutine());
    }
    ...
    private IEnumerator PlayLettreICoroutine()
    {
        if (rightHandTarget == null)
        {
            Debug.LogError("[LETTRE I V1] RightHandTarget non assigné.");
            yield break;
        }

        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null)
        {
            Debug.LogError("[LETTRE I V1] AvatarRoot requis.");
            yield break;
        }

        if (showDebugLogs) Debug.Log("[LETTRE I V1] Démarrage.");
        ...
        if (showDebugLogs) Debug.Log("[LETTRE I V1] Position I maintenue.");
    ...
    private void ApplyBone(string boneName, Quaternion rotation)
    {
        ...
        if (bone != null)
            ...
            Debug.LogWarning("[LETTRE I V1] Os introuvable : " + boneName);
    }
