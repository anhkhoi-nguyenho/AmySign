...
/// <summary>
/// Script Unity pour la lettre N en LSF.
/// Généré depuis lettre_n_output_gestes_unity_1m60_v1.json.
/// 
/// Forme de main :
/// - Index et majeur tendus.
/// - Annulaire, auriculaire et pouce repliés.
/// - Pose maintenue sans retour au repos.
/// 
/// À utiliser comme la lettre T :
/// - Assigner RightHandTarget
/// - Assigner RightElbowHint
/// - Assigner HeadTarget si disponible
/// - Assigner AvatarRoot
/// </summary>
public class LettreNUnityPlayer_1m60_V1 : MonoBehaviour
{
    ...
    [Header("Poignet — appliqué une seule fois à la fin")]
    ...
    private void Start()
    {
        if (playOnStart) PlayLettreN();
    }

    public void PlayLettreN()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayLettreNCoroutine());
    }
    ...
    private IEnumerator PlayLettreNCoroutine()
    {
        if (rightHandTarget == null)
        {
            Debug.LogError("[LETTRE N V1] RightHandTarget non assigné.");
            yield break;
        }

        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null)
        {
            Debug.LogError("[LETTRE N V1] AvatarRoot requis.");
            yield break;
        }

        if (showDebugLogs) Debug.Log("[LETTRE N V1] Démarrage.");
        ...
        if (showDebugLogs) Debug.Log("[LETTRE N V1] Position N maintenue.");
    ...
    private void ApplyBone(string boneName, Quaternion rotation)
    {
        ...
        if (bone != null)
            ...
            Debug.LogWarning("[LETTRE N V1] Os introuvable : " + boneName);
    }
