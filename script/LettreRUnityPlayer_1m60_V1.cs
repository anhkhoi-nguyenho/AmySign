...
/// <summary>
/// Script Unity pour la lettre R en LSF.
/// Généré depuis lettre_r_output_gestes_unity_1m60_v1.json.
/// Index et majeur tendus/croisés ; annulaire et auriculaire repliés ; pouce posé sur l'annulaire.
/// Pose maintenue sans retour au repos.
/// </summary>
public class LettreRUnityPlayer_1m60_V1 : MonoBehaviour
{
    ...
    public Vector3 wristRotationEuler = new Vector3(0f, 160f, -30f);
    public bool useLocalRotation = true;

    [Header("Doigts — activer")]
    public bool forceManualHandshape = true;

    [Header("Index — tendu, incliné pour croisement")]
    public Vector3 indexPh1 = new Vector3(0f, 0f, -12f);
    public Vector3 indexPh2 = new Vector3(0f, 0f, 0f);
    public Vector3 indexPh3 = new Vector3(0f, 0f, 0f);

    [Header("Majeur — tendu, croisé")]
    public Vector3 middlePh1 = new Vector3(0f, 0f, 18f);
    public Vector3 middlePh2 = new Vector3(8f, 0f, 0f);
    public Vector3 middlePh3 = new Vector3(5f, 0f, 0f);
    ...
    [Header("Pouce — posé sur l'annulaire")]
    public Vector3 thumbPh1 = new Vector3(45f, 25f, 45f);
    public Vector3 thumbPh2 = new Vector3(40f, 0f, 10f);
    public Vector3 thumbPh3 = new Vector3(20f, 0f, 0f);

    [Header("Debug")]
    public bool showDebugLogs = true;

    [Header("Position finale — modifiable")]
    public Vector3 handFinalPosition = new Vector3(0.12f, 1.05f, 0.16f);
    public Vector3 elbowFinalPosition = new Vector3(0.32f, 0.82f, 0.1f);
    ...
    private void Start()
    {
        if (playOnStart) PlayLettreR();
    }

    public void PlayLettreR()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayLettreRCoroutine());
    }
    ...
    private IEnumerator PlayLettreRCoroutine()
    {
        if (rightHandTarget == null)
        {
            Debug.LogError("[LETTRE R V1] RightHandTarget non assigné.");
            yield break;
        }

        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null)
        {
            Debug.LogError("[LETTRE R V1] AvatarRoot requis.");
            yield break;
        }

        if (showDebugLogs) Debug.Log("[LETTRE R V1] Démarrage.");
        ...
        if (showDebugLogs) Debug.Log("[LETTRE R V1] Position R maintenue.");
    ...
    private void ApplyBone(string boneName, Quaternion rotation)
    {
        ...
        if (bone != null)
            ...
            Debug.LogWarning("[LETTRE R V1] Os introuvable : " + boneName);
    }
