...
/// <summary>
/// Script Unity pour le signe COMPRENDRE en LSF.
/// Main près de la tempe, index+majeur en pince avec le pouce, annulaire+auriculaire repliés.
/// </summary>
public class COMPRENDRE : MonoBehaviour
{
    ...
    [Header("Position COMPRENDRE — proche de la tempe")]
    [Tooltip("X=0.15 (côté droit), Y=1.35 (niveau tempe), Z=0.05 (proche du visage)")]
    public Vector3 handFinalPosition = new Vector3(0.15f, 1.35f, 0.05f);
    [Tooltip("Coude vers le bas-arrière pour que le bras soit plié")]
    public Vector3 elbowFinalPosition = new Vector3(0.35f, 0.95f, -0.08f);

    [Header("Poignet")]
    public bool applyWristRotation = true;
    public Vector3 wristRotationEuler = new Vector3(0f, 180f, -90f);
    ...
    public Vector3 indexPh2 = new Vector3(70f, 0f, 0f);
    public Vector3 indexPh3 = new Vector3(30f, 0f, 0f);

    [Header("Majeur — pince")]
    public Vector3 middlePh1 = new Vector3(60f, 0f, 0f);
    public Vector3 middlePh2 = new Vector3(70f, 0f, 0f);
    ...
    [Header("Pouce — pince")]
    public Vector3 thumbPh1 = new Vector3(20f, 30f, 40f);
    public Vector3 thumbPh2 = new Vector3(30f, 0f, 20f);
    ...
    private void Start()
    {
        if (playOnStart) PlayComprendre();
    }

    public void PlayComprendre()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayComprendreCoroutine());
    }

    private IEnumerator PlayComprendreCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[COMPRENDRE] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[COMPRENDRE] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[COMPRENDRE] Démarrage.");
        ...
        if (showDebugLogs) Debug.Log("[COMPRENDRE] Position maintenue.");
