...
/// <summary>
/// Script Unity pour la lettre U en LSF.
/// Index + majeur tendus collés, annulaire + auriculaire repliés, pouce sur annulaire.
...
public class U : MonoBehaviour
{
    ...
    [Tooltip("Y=0 pour paume vers la caméra")]
    ...
    private void Start()
    {
        if (playOnStart) PlayU();
    }

    public void PlayU()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayUCoroutine());
    }

    private IEnumerator PlayUCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[U] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[U] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[U] Démarrage.");
