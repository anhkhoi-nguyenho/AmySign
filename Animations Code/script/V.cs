...
/// <summary>
/// Script Unity pour la lettre V en LSF.
/// Index et majeur tendus écartés en V, annulaire + auriculaire repliés, pouce dessus.
...
public class V : MonoBehaviour
{
    ...
    [Header("Index — tendu incliné gauche (forme du V)")]
    public Vector3 indexPh1 = new Vector3(-20f, -15f, 0f);
    public Vector3 indexPh2 = new Vector3(-15f, 0f, 0f);
    public Vector3 indexPh3 = new Vector3(-10f, 0f, 0f);

    [Header("Majeur — tendu incliné droite (forme du V)")]
    public Vector3 middlePh1 = new Vector3(-20f, 15f, 0f);
    ...
    private void Start()
    {
        if (playOnStart) PlayV();
    }

    public void PlayV()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayVCoroutine());
    }

    private IEnumerator PlayVCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[V] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[V] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[V] Démarrage.");
