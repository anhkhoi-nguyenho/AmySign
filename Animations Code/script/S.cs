...
/// <summary>
/// Script Unity pour la lettre S en LSF.
/// Poing fermé, tous les doigts pliés, pouce posé sur l'index et le majeur.
...
public class S : MonoBehaviour
{
    ...
    [Tooltip("Y=0 pour paume vers la caméra (différent des autres lettres)")]
    public Vector3 wristRotationEuler = new Vector3(0f, 0f, 0f);
    public bool useLocalRotation = true;

    [Header("Doigts — tous pliés en poing")]
    ...
    [Header("Pouce — posé sur index et majeur")]
    ...
    private void Start()
    {
        if (playOnStart) PlayS();
    }

    public void PlayS()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlaySCoroutine());
    }

    private IEnumerator PlaySCoroutine()
    {
        if (rightHandTarget == null) { Debug.LogError("[S] RightHandTarget non assigné."); yield break; }
        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null) { Debug.LogError("[S] AvatarRoot requis."); yield break; }

        if (showDebugLogs) Debug.Log("[S] Démarrage.");
