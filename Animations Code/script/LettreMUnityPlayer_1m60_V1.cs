...
/// <summary>
/// Script Unity pour la lettre M en LSF.
/// Généré depuis lettre_m_output_gestes_unity_1m60_v1.json.
/// Index, majeur, annulaire et auriculaire tendus ; pouce replié et caché.
/// Pose maintenue sans retour au repos.
/// </summary>
public class LettreMUnityPlayer_1m60_V1 : MonoBehaviour
{
    ...
    public Vector3 wristRotationEuler = new Vector3(0f, 340f, -90f);
    ...
    [Header("Pouce — replié / caché")]
    public Vector3 thumbPh1 = new Vector3(50f, 30f, 60f);
    public Vector3 thumbPh2 = new Vector3(50f, 0f, 10f);
    public Vector3 thumbPh3 = new Vector3(30f, 0f, 0f);
    ...
    private void Start()
    {
        if (playOnStart) PlayLettreM();
    }

    public void PlayLettreM()
    {
        if (currentCoroutine != null) StopCoroutine(currentCoroutine);
        currentCoroutine = StartCoroutine(PlayLettreMCoroutine());
    }
    ...
    private IEnumerator PlayLettreMCoroutine()
    {
        if (rightHandTarget == null)
        {
            Debug.LogError("[LETTRE M V1] RightHandTarget non assigné.");
            yield break;
        }

        if (targetPositionSpace == TargetPositionSpace.RelativeToAvatarRoot && avatarRoot == null)
        {
            Debug.LogError("[LETTRE M V1] AvatarRoot requis.");
            yield break;
        }

        if (showDebugLogs) Debug.Log("[LETTRE M V1] Démarrage.");
        ...
        if (showDebugLogs) Debug.Log("[LETTRE M V1] Position M maintenue.");
    ...
    private void ApplyBone(string boneName, Quaternion rotation)
    {
        ...
        if (bone != null)
            ...
            Debug.LogWarning("[LETTRE M V1] Os introuvable : " + boneName);
    }
